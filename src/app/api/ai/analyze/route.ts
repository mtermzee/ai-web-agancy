...
import {
	analyzeCompanyWithGemini,
	chooseGeminiAnalysisMode,
	isGeminiConfigured,
...
export async function POST(request: Request) {
	...
	if (!membership) {
		return NextResponse.json(
			{
				error: "Workspace access denied for this account.",
	...
	if (!isGeminiConfigured()) {
		return NextResponse.json(
			{
				error:
					"Gemini is not configured. Add GEMINI_API_KEY in Vercel → Settings → Environment Variables and redeploy.",
	...
	try {
		...
		return NextResponse.json(
			{ error: "Invalid request body", code: "INVALID_REQUEST", requestId },
			{ status: 400 },
		);
	}
		...
		return NextResponse.json(
			{
				error: companyResult.error
					? `Company lookup failed: ${companyResult.error.message}`
					: "Company not found",
	...
	if (sourceErrors.length) {
		...
		return NextResponse.json(
			{
				error: `Supabase read failed: ${detail}`,
	...
	try {
		...
		if (auditSave.error)
			throw new Error(`Save audit: ${auditSave.error.message}`);
		...
		if (leadSave.error)
			throw new Error(`Save lead score: ${leadSave.error.message}`);
		...
		const activitySave = await supabase.from("lead_activities").insert({
			...
			detail: `${result.model} · ${result.mode === "website_url_context" ? "website URL context" : result.mode === "stored_profile" ? "stored profile fallback" : "business data only"} · confidence ${analysis.confidence}%`,
			created_at: result.analyzedAt,
		});
		if (activitySave.error)
			throw new Error(`Save activity: ${activitySave.error.message}`);
		...
		console.error(`[AgencyOS AI ${requestId}]`, message);
		return NextResponse.json(
			{ error: message, code: "AI_ANALYSIS_FAILED", requestId },
			{ status: 500 },
		);
	}
}
