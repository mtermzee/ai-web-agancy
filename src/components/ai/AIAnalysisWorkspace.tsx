...
export function AIAnalysisWorkspace() {
	...
	const rows = useMemo(() => {
		...
		return [...companies]
			...
			.sort(
				(a, b) =>
					(getWorkflow(b.id)?.leadScore ?? 0) -
					(getWorkflow(a.id)?.leadScore ?? 0),
			);
		...
		<div className="page">
			<div className="page-header">
				<div>
					...
					<p className="page-subtitle">
						Run human-triggered Gemini audits and persist the structured result
						to Supabase.
					</p>
				</div>
			</div>
			...
			<section className="card ai-analysis-intro">
				...
				<div>
					...
					<p>
						Real public websites use Gemini URL Context. Demo URLs such as{" "}
						<code>.example</code> automatically use the stored audit profile
						instead, so the prototype no longer fails on fictional websites.
						Companies without a website use business data only.
					</p>
				</div>
			</section>
							...
							{rows.map((company) => {
								...
								return (
									<tr key={company.id}>
										...
										<td>
											{company.hasWebsite &&
											isPublicHttpUrl(company.website) ? (
												<span className="analysis-mode url">URL Context</span>
											) : company.hasWebsite ? (
												<span className="analysis-mode stored">
													Stored profile
												</span>
											) : (
												<span className="analysis-mode data">
													Business data
												</span>
											)}
										</td>
										<td>
											<ScoreChip score={company.scores.overall} />
										</td>
										<td>
											<span className="lead-score-chip compact">
												{workflow?.leadScore ?? 0}
											</span>
										</td>
										<td className="muted-cell">
											{formatAnalyzedAt(company.lastAnalyzedAt)}
										</td>
