...
export function DashboardClient() {
  ...
  const avgWebsiteScore = Math.round(companies.reduce((sum, company) => sum + company.scores.overall, 0) / total);
  const avgLeadScore = Math.round(companies.reduce((sum, company) => sum + (getWorkflow(company.id)?.leadScore ?? 0), 0) / total);
  ...
  return <div className="page">
    <div className="page-header"><div><div className="eyebrow">AI lead intelligence</div><h1>Dashboard</h1><p className="page-subtitle">Sprint 0.3 adds local CRM state, lead scoring, notes, activity history and outreach approval.</p></div><Link href="/companies" className="button primary"><Building2 size={17}/>Browse companies</Link></div>
    <section className="kpi-grid">
      ...
      <KpiCard label="Avg. website score" value={`${avgWebsiteScore}/100`} change="portfolio average" icon={Activity}/>
      <KpiCard label="Avg. lead score" value={`${avgLeadScore}/100`} change="local scoring model" icon={Gauge}/>
      <KpiCard label="Mockups ready" value={mockups} change="local workflow state" icon={LayoutTemplate}/>
      <KpiCard label="Review queue" value={reviewCount} change="needs human action" icon={Target}/>
      <KpiCard label="Approved drafts" value={approvedDrafts} change="still not sent" icon={CheckCircle2}/>
    </section>
    <div className="dashboard-grid">
      <section className="card panel"><div className="panel-header"><div><h2>Top opportunities</h2><div className="panel-note">Ranked by the new MVP lead score</div></div><Link className="button secondary" href="/leads">Review leads</Link></div><div className="lead-list">
